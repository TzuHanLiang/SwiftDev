...
class SettingsViewController: NSViewController, NSCollectionViewDataSource, NSCollectionViewDelegate, NSCollectionViewDelegateFlowLayout {
    ...
    func rename() {
        ...
        for wallet in hdw.wallets ?? [] {
            items.append(wallet.name)
            let size = wallet.name.sizeOfFont(NSFont.systemFont(ofSize: 13))
        ...
        let okAction = AppController.AlertAction(title: "ok".localizedString) {
            ...
            let okAction = AppController.AlertAction(title: "ok".localizedString, callback: {
                ...
                if index == 0 {
                    ...
                    AppController.shared.coldWalletChangeHDWalletName(newName) { (succeeded, error) in
                        ...
                        AppController.shared.showAlert("succeeded_to_change_name".localizedString)
                    }
                }
            ...
            AppController.shared.showAlert(currentName, nil, [okAction, cancelAction], textField)
        }
        AppController.shared.showAlert("select_one_to_rename".localizedString, nil, [okAction, cancelAction], popUpButton)
    }
    ...
    func bindLoginFingerprint() {
        class Delegate: FingerprintVerificationViewControllerDelegate {
            ...
            func fpVerificationDidComplete(_ vc: FingerprintVerificationViewController, _ done: Bool, _ verified: Bool) {
                ...
                AppController.shared.showBusyPrompt(self.parent)
                self.cancelCallback = AppController.shared.coldWalletFinishVerifyingBoundLoginFingerprint(false) { (succeeded, error) in
                    ...
                    AppController.shared.coldWalletFinishBindingLoginFingerprint(false) { (succeeded, error) in
                        ...
                        AppController.shared.showAlert("bind_succeeded".localizedString)
                    }
                }
            }
        }
    ...
    func unbindLoginFingerprint() {
        let yesAction = AppController.AlertAction(title: "yes".localizedString) {
            ...
            AppController.shared.coldWalletUnbindLoginFingerprint { (succeeded, error) in
                ...
                AppController.shared.showAlert("login_fp_have_been_unbound".localizedString)
            }
        }
        let noAction = AppController.AlertAction(title: "no".localizedString, callback: nil)
        AppController.shared.showAlert("unbind_login_fp_check_msg".localizedString, nil, [yesAction, noAction])
    }
    ...
    func addCryptocurrency() {
        ...
        let okAction = AppController.AlertAction(title: "ok".localizedString) {
            ...
            AppController.shared.coldWalletAddNewCryptocurrency(nil, cryptocurrencyType, nil, nil, nil) { (succeeded, error) in
                AppController.shared.hideBusyPrompt(self)
                guard succeeded, error == nil else {
                    (error != nil) ? ATLog.debug(error!.description) : nil
                    switch error {
        ...
        let advancedAction = AppController.AlertAction(title: "advanced".localizedString) {
            ...
            //guard let view = topLevelArray?.firstObject as? AdvancedCryptocurrencyInfoView else { return }
            view.nickNameTextField.placeholderString = "nickname".localizedString
            view.nickNameTextField.stringValue = ""
            view.purposeTextField.placeholderString = "Purpose'"
            view.purposeTextField.stringValue = "44'"
            ...
            let okAction = AppController.AlertAction(title: "ok".localizedString, callback: {
                let name = (view.nickNameTextField.stringValue != "") ? view.nickNameTextField.stringValue : nil
                let purpose = (view.purposeTextField.stringValue != "") ? UInt32(view.purposeTextField.stringValue.replacingOccurrences(of: "'", with: "")) : nil
            ...
            AppController.shared.showAlert(cryptocurrencyType.name, nil, [okAction, cancelAction], view)
        }
        AppController.shared.showAlert("select_one_to_add".localizedString, nil, [okAction, advancedAction, cancelAction], popUpButton)
    }
        ...
        let okAction = AppController.AlertAction(title: "ok".localizedString) {
            ...
            let yesAction = AppController.AlertAction(title: "delete".localizedString, callback: {
                ...
                AppController.shared.coldWalletDeleteCryptocurrency(cryptocurrency) { (succeeded, error) in
                    ...
                    AppController.shared.showAlert("succeeded_to_delete".localizedString)
                }
            })
            let noAction = AppController.AlertAction(title: "cancel".localizedString, callback: nil)
            AppController.shared.showAlert("delete_account_warning".localizedString, "\(cryptocurrency.name)\nm / \(cryptocurrency.purpose & 0x7FFFFFFF)' / \(cryptocurrency.coinType & 0x7FFFFFFF)' / \(cryptocurrency.accountValue & 0x7FFFFFFF)'", [yesAction, noAction])
        }
        let cancelAction = AppController.AlertAction(title: "cancel".localizedString, callback: nil)
        AppController.shared.showAlert("select_one_to_delete".localizedString, nil, [okAction, cancelAction], popUpButton)
    }
        ...
        struct Delegate: FingerprintVerificationViewControllerDelegate {
            ...
            func fpVerificationDidComplete(_ vc: FingerprintVerificationViewController, _ done: Bool, _ verified: Bool) {
                ...
                AppController.shared.coldWalletFinishFingerprintDeletion({ (succeeded, fpids, error) in
                    ...
                    AppController.shared.showAlert(message)
                })
            }
        }
    ...
    func calibrateFingerprintSensor() {
        AppController.shared.showAlert("calibration_warning".localizedString, nil, [AppController.AlertAction(title: "ok".localizedString, callback: {
            ...
            AppController.shared.coldWalletCalibrateFingerprintSensor { (succeeded, error) in
                ...
                AppController.shared.showAlert("succeeded_to_calibrate_fp_sensor".localizedString)
            }
        })])
    }
    ...
    func updateFirmware() {
        ...
        AppController.shared.checkNewUpdates { (fw, cos) in
            ...
            guard fw != nil || cos != nil else {
                AppController.shared.showAlert("firmware_is_update_to_date".localizedString)
                return
            }
                        ...
                        let cosDownloadTask = URLSession.shared.dataTask(with: cosUrl) { (data, response, error) in
                            ...
                            AppController.shared.coldWalletUpdateFirmwareAndCos(fwData, cosData) { (progress, error) in
                                ...
                                guard progress < 100 else {
                                    AppController.shared.hideBusyPrompt(self)
                                    AppController.shared.showAlert("update_succeeded_description".localizedString)
                                    return
                                }
            ...
            let noAction = AppController.AlertAction(title: "no".localizedString, callback: nil)
            AppController.shared.showAlert("firmware_update_description".localizedString, "\((newFwAvailable) ? "FW: \(newFwVersion)\n" : "")\(newCosAvailable ? "COS: \(newCosVersion)\n" : "")", [yesAction, noAction])
        }
    }
    ...
    func resetWallet() {
        struct Delegate: FingerprintVerificationViewControllerDelegate {
            ...
            func fpVerificationDidComplete(_ vc: FingerprintVerificationViewController, _ done: Bool, _ verified: Bool) {
                ...
                AppController.shared.coldWalletResetHDWallet { (done, error) in
                    ...
                    AppController.shared.showAlert("reset_complete_description".localizedString, nil, [AppController.AlertAction(title: "ok".localizedString, callback: {
                        AppController.shared.popSplitDetailViewToRootViewController()
                    })])
                }
            }
        ...
        let noAction = AppController.AlertAction(title: "no".localizedString, callback: nil)
        AppController.shared.showAlert("reset_wallet_description".localizedString, nil, [yesAction, noAction])
    }
    ...
    func resetToFactory() {
        ...
        let noAction = AppController.AlertAction(title: "no".localizedString, callback: nil)
        AppController.shared.showAlert("factory_reset_description".localizedString, nil, [yesAction, noAction])
    }
    ...
    func showDeviceInformation() {
        ...
        AppController.shared.coldWalletGetDeviceInformationDescription { (description, error) in
            ...
            AppController.shared.showAlert(description)
        }
    }
    ...
    func changeDisplayCurrency() {
        ...
        let popUpButton = NSPopUpButton(frame: NSMakeRect(0, 0, width + 40, height + 10))
        popUpButton.addItems(withTitles: items)
        if let index = ExchangeRate.currencies.firstIndex(of: AppConfig.shared.defaultCurrencyUnit) {
            popUpButton.selectItem(at: index)
        }
        let okAction = AppController.AlertAction(title: "ok".localizedString) {
            let index = popUpButton.indexOfSelectedItem
            AppConfig.shared.defaultCurrencyUnit = ExchangeRate.currencies[index]
            for wallet in AppController.shared.coldWalletGetHDWallet()?.wallets ?? [] {
                ExchangeRate(.coinbase).cryptocurrencyToCurrency(wallet.currencyType, AppConfig.shared.defaultCurrencyUnit) { (rate) in
                    wallet.exchangeRates[AppConfig.shared.defaultCurrencyUnit.description] = rate
                }
            }
        }
        let cancelAction = AppController.AlertAction(title: "cancel".localizedString, callback: nil)
        AppController.shared.showAlert("select_currency".localizedString, nil, [okAction, cancelAction], popUpButton)
    }
    ...
    func collectionView(_ collectionView: NSCollectionView, didSelectItemsAt indexPaths: Set<IndexPath>) {
        ...
        if functionName == "bind_login_fingerprint", !bindingFingerprintEnabled {
            ATLog.info("\(TapCounter.continuousTapTimes)")
            if TapCounter.continuousTapTimes >= 20 {
                ATLog.info("enable binding fp function")
