...
class WalletViewController: NSViewController, NSTableViewDataSource, NSTableViewDelegate  {
    ...
    @IBAction func addButtonAction(_ sender: NSButton) {
        ...
        AppController.shared.showAlert("select_one_to_add".localizedString, nil, [okAction, cancelAction], popUpButton)
    }
    ...
    func updateCurrencyAmount() {
        ...
        self.currencyAmountLabel.stringValue = "\(amount.toString(2)) \(defaultCurrencyUnit.description)"
    }
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        if let rate = wallet.exchangeRates[currencyUnit.description] {
            view?.exchangeRateLabel.stringValue = "1 \(wallet.currencyType.symbol) = \(rate.toString(2)) \(currencyUnit.description)"
            if let balance = Double(wallet.balanceString) {
                view?.currencyAmountLabel.stringValue = "\((balance * rate).toString(2)) \(currencyUnit.description)"
            }
        }
        if wallet.isSyncing {
            view?.currencyAmountLabel.stringValue = "synchronizing".localizedString
