...
class SettingsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func rename() {
        ...
        pickerView.frame = CGRect(x: 0, y: 0, width: 250, height: height)
        pickerView.reloadAllComponents()
        AppController.shared.showAlert(self, "select_one_to_rename".localizedString, nil, [okAction, cancelAction], pickerView)
    }
    ...
    func addCryptocurrency() {
        class TextFieldDelegate: NSObject, UITextFieldDelegate {
            func textFieldDidEndEditing(_ textField: UITextField) {
                if textField.accessibilityIdentifier == "purposeTextField" {
                    guard let text = textField.text, let purpose = UInt32(text.replacingOccurrences(of: "'", with: "")) else {
                        textField.text = "44'"
                        return
                    }
        ...
        let advancedAction = UIAlertAction(title: "advanced".localizedString, style: .default) { (action) in
            ...
            DispatchQueue.main.async {
                ...
                alert.addAction(UIAlertAction(title: "ok".localizedString, style: .default, handler: { (action) in
                    ...
                    let purpose = UInt32(alert.textFields?[1].text?.replacingOccurrences(of: "'", with: "") ?? "44")
                    let account = (alert.textFields?[3].text != nil) ? UInt32(alert.textFields![3].text!.replacingOccurrences(of: "'", with: "")) : nil
                    let dateFormatter = DateFormatter()
                    dateFormatter.dateFormat = "yyyy-MM-dd"
        ...
        pickerView.frame = CGRect(x: 0, y: 0, width: 250, height: height)
        pickerView.reloadAllComponents()
        AppController.shared.showAlert(self, "select_one_to_add".localizedString, nil, [okAction, advancedAction, cancelAction], pickerView)
    }
    ...
    func deleteCryptocurrency() {
        ...
        pickerView.frame = CGRect(x: 0, y: 0, width: 250, height: height)
        pickerView.reloadAllComponents()
        AppController.shared.showAlert(self, "select_one_to_delete".localizedString, nil, [okAction, cancelAction], pickerView)
    }
    ...
    func changeDisplayCurrency() {
        self.pickerViewItems = ExchangeRate.currencies
        ...
        pickerView.frame = CGRect(x: 0, y: 0, width: 250, height: height)
        pickerView.reloadAllComponents()
        AppController.shared.showAlert(self, "select_currency".localizedString, nil, [okAction, cancelAction], pickerView)
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        else if abs(TapCounter.timestamp.timeIntervalSinceNow) > 0.5 {
            TapCounter.timestamp = Date()
            TapCounter.continuousTapTimes = 1
        }
        else {
