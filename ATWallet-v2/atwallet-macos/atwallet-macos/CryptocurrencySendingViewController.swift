...
class CryptocurrencySendingViewController: NSViewController, NSTextFieldDelegate {
    ...
    @IBOutlet weak var maxAmountLabel: NSTextField!
    ...
    @IBAction func lowSpeedButtonAction(_ sender: NSButton) {
        ...
        self.feeTextField.stringValue = cryptocurrency.calculateLowFee(self.amountTextField.stringValue, self.noteTextField.stringValue, self.token)
    }
    
    @IBAction func mediumSpeedButtonAction(_ sender: NSButton) {
        guard  let cryptocurrency = self.cryptocurrency, self.amountTextField.stringValue.count > 0 else { return }
        self.feeTextField.stringValue = cryptocurrency.calculateMediumFee(self.amountTextField.stringValue, self.noteTextField.stringValue, self.token)
    }
    
    @IBAction func highSpeedButtonAction(_ sender: NSButton) {
        guard  let cryptocurrency = self.cryptocurrency, self.amountTextField.stringValue.count > 0 else { return }
        self.feeTextField.stringValue = cryptocurrency.calculateHighFee(self.amountTextField.stringValue, self.noteTextField.stringValue, self.token)
    }
    ...
    @IBAction func sendButtonAction(_ sender: NSButton) {
        ...
        guard maxAmountValue > 0 else {
            AppController.shared.showAlert("failed_to_create_transaction_description".localizedString)
            return
        }
        guard amountValue <= maxAmountValue else {
            AppController.shared.showAlert("amount_is_too_high".localizedString)
            return
        }
        ...
        let minFeeValue = Double(cryptocurrency.calculateMinimumFee(amount, self.noteTextField.stringValue, self.token)) ?? 0
        guard feeValue >= minFeeValue else {
            AppController.shared.showAlert((self.token != nil && self.cryptocurrency?.currencyType == .eth) ? "gas_price_is_too_small".localizedString : "fee_is_too_small".localizedString)
            return
        }
        let balanceValue = Double(cryptocurrency.balanceString) ?? 0
        if self.token == nil {
            guard (amountValue + feeValue) <= balanceValue else {
                AppController.shared.showAlert("no_enough_balance_for_amount_and_fee".localizedString)
                return
            }
            guard feeValue < amountValue else {
                AppController.shared.showAlert("fee_is_greater_than_amount".localizedString)
                return
            }
        }
                ...
                if done {
                    ...
                    AppController.shared.coldWalletSignTransaction(cryptocurrency, transaction) { (succeeded, transaction, error) in
                        ...
                        AppController.shared.coldWalletPublishTransaction(cryptocurrency, transaction) { (succeeded, transaction, error) in
                            ...
                            self.parent.transaction = nil
                            if succeeded {
                                AppController.shared.showInformation("transaction_succeeded".localizedString, nil, [AppController.AlertAction(title: "ok".localizedString, callback: {
                                    AppController.shared.popSplitDetailView()
                                })])
                            }
                            else {
        ...
        AppController.shared.coldWalletPrepareToSignTransaction(cryptocurrency, transaction) { (succeeded, transaction, error) in
            ...
            """
            if self.noteTextField.stringValue.count > 0 {
                let noteDescription = """
            ...
            AppController.shared.showInformation(confirmationMessage, transactionDescription, [confirmAction, cancelAction])
        }
    }
    ...
    override func viewDidLoad() {
        ...
        self.maxAmountLabel.stringValue = ""
    ...
    override func viewWillAppear() {
        ...
        self.titleLabel.stringValue = "\("send".localizedString) \(self.token?.info.symbol ?? self.cryptocurrency?.currencyType.symbol ?? "cryptocurrency".localizedString)"
        self.addressTextField.stringValue = ""
        self.addressTextField.placeholderString = nil
        self.amountTextField.stringValue = ""
        self.amountTextField.placeholderString = self.token?.info.symbol ?? self.cryptocurrency?.currencyType.symbol ?? ""
        self.feeLabel.stringValue = (self.token != nil && self.cryptocurrency?.currencyType == .eth) ? "gas_price".localizedString : "fee".localizedString
        self.feeTextField.stringValue = ""
        self.feeTextField.placeholderString = (self.token != nil && self.cryptocurrency?.currencyType == .eth) ? "GWEI" : cryptocurrency?.currencyType.symbol
        self.noteTextField.stringValue = ""
        ...
        if let symbol = self.token?.info.symbol, let maxAmount = self.token?.balanceString {
            self.maxAmountLabel.stringValue = "( \("max_amount".localizedString): \(maxAmount) \(symbol) )"
        }
        else if self.token == nil, let symbol = self.cryptocurrency?.currencyType.symbol, let maxAmount = self.cryptocurrency?.getMaxOutputAmount() {
            self.maxAmountLabel.stringValue = "( \("max_amount".localizedString): \(maxAmount) \(symbol) )"
        }
    }
    ...
    func controlTextDidEndEditing(_ obj: Notification) {
        ...
        if textField == self.amountTextField || textField == self.noteTextField {
            ...
            let minFeeStr = cryptocurrency.calculateMinimumFee(amountStr, self.noteTextField.stringValue, self.token)
            let medFeeStr = cryptocurrency.calculateMediumFee(amountStr, self.noteTextField.stringValue, self.token)
            ...
            guard feeStr.isNumeric else {
                AppController.shared.showAlert((self.token != nil && self.cryptocurrency?.currencyType == .eth) ? "invalid_gas_price".localizedString : "invalid_fee".localizedString)
                textField.stringValue = ""
                return
            }
        }
