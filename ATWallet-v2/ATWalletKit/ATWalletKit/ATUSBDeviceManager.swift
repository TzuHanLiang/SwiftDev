...
public class ATUSBDeviceManager : ATDeviceManager {
    ...
    private override init() {
        ...
        let matchingCallback: IOHIDDeviceCallback = { context, result, sender, device in
            ATLog.verbose("Found USB device: \(IOHIDDeviceGetProperty(device, kIOHIDProductKey as CFString) as? String ?? "unknown")")
        ...
        let removalCallback: IOHIDDeviceCallback = { context, result, sender, device in
            ATLog.verbose("Removed USB device: \(IOHIDDeviceGetProperty(device, kIOHIDProductKey as CFString) as? String ?? "unknown")")
            let usbDeviceManager: ATUSBDeviceManager = unsafeBitCast(context, to: ATUSBDeviceManager.self)
            let usbDevice = ATUSBDevice(HIDDevice: device, DeviceManager: usbDeviceManager)
    ...
    func contains(_ device: ATUSBDevice) -> Bool {
        return self.usbDevices.contains(device)
    }
    
    func popDevice(_ serialNumber: String) -> ATUSBDevice? {
        guard serialNumber.count > 0 else { return nil }
        var atUsbDevice: ATUSBDevice?
        for index in 0..<self.usbDevices.count {
            let device = self.usbDevices[index]
            if device.serialNumber == serialNumber {
                atUsbDevice = device
                self.usbDevices.remove(at: index)
                break
            }
        }
        return atUsbDevice
    ...
    func pushDevice(_ device: ATUSBDevice) {
        self.usbDevices.append(device)
    }
