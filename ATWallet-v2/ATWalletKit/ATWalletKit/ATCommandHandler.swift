    ...
    private var currentCommand: ATCommand {
        get {
            if let cmd = self.currentItem as? ATCommand {
                ...
                let cmd = ATCommand { (data, error) in
                    // do nothing
                }
                return cmd
            }
    ...
    func sessionDidFailToCreate(_ error: ATError) {
        self.dispatchQueue.async {
            ...
            while let item = self.dequeue() {
                if let cmd = item as? ATCommand {
                    cmd.resultHandler(nil, .failToSetupSession)
                }
                else if let bundle = item as? ATCommandBundle {
                    bundle.command?.resultHandler(nil, .failToSetupSession)
                }
            }
        }
    }
