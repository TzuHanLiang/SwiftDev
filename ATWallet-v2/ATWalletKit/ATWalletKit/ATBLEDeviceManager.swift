...
public class ATBLEDeviceManager : ATDeviceManager, CBCentralManagerDelegate {
    ...
    public override func connect(_ device: ATDevice) {
        ...
        DispatchQueue.main.async {
            ...
            let block: (Timer) -> Void = { (timer) in
                count += 1
                if !bleDevice.connected {
                    if bleDevice.peripheral.state == .connected, count < 5 {
                        // in pairing
                        DispatchQueue.main.async {
                            Timer.scheduledTimer(withTimeInterval: 5, repeats: false, block: timerCallback)
                        }
                    }
                    else if bleDevice.peripheral.state != .connected {
    ...
    public func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        ATLog.verbose("Found BLE device: \(peripheral.name ?? "unknown"), \(RSSI)")
        ...
        if isATWallet && peripheral.name != nil {
            ...
            if bleDevice == nil {
                ...
                self.foundDevices?.append(bleDevice!) ?? (foundDevices = [bleDevice!])
            }
            DispatchQueue.main.async {
