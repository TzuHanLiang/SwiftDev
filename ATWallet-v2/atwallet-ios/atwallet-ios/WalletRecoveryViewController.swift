...
class WalletRecoveryViewController: UIViewController, UITextFieldDelegate, UITableViewDataSource, UITableViewDelegate, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    @IBAction func textFieldAction(_ sender: UITextField) {
        ...
        else if sender.accessibilityIdentifier == "purposeTextField" {
            self.pickerType = .purpose
            height = 70 * self.segwitPurposeItems.count
        }
        else {
            return
        }
        ...
        let action = UIAlertAction(title: NSLocalizedString("ok", comment: ""), style: .default) { (action) in
            ...
            case .cryptocurrencyType:
                ...
                if self.recoveryCryptocurrencies.count > section {
                    ...
                    var purpose = self.purposeValues["44'"]
                    (old.currency == .btc || old.currency == .ltc) ? purpose = old.purpose : nil
                    self.recoveryCryptocurrencies[section] = ATColdWallet.CurrencyWalletRecoveryInfo(Purpose: purpose, Currency: cryptocurrencyType, Account: old.account, Timestamp: old.timestamp, Name: (old.name == old.currency.name) ? cryptocurrencyType.name : old.name)
                }
                else {
            ...
            case .purpose:
                if self.recoveryCryptocurrencies.count > section {
                    let old = self.recoveryCryptocurrencies[section]
                    var purpose = self.purposeValues["44'"]
                    if (old.currency == .btc || old.currency == .ltc) && index < self.segwitPurposeItems.count {
                        purpose = self.purposeValues[self.segwitPurposeItems[index]]
                    }
                    self.recoveryCryptocurrencies[section] = ATColdWallet.CurrencyWalletRecoveryInfo(Purpose: purpose, Currency: old.currency, Account: old.account, Timestamp: old.timestamp, Name: old.name)
                }
            }
            self.recoveryTableView.reloadData()
    ...
    */
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        if textField.accessibilityIdentifier == "cryptocurrencyTypeTextField" || textField.accessibilityIdentifier == "yearTextField" {
            ...
            self.view.endEditing(true)
            if textField.tag < self.recoveryCryptocurrencies.count {
                let old = self.recoveryCryptocurrencies[textField.tag]
                (old.currency == .btc || old.currency == .ltc) ? textFieldAction(textField) : nil
            }
            return false
        }
        return true
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0 {
            ...
            if AppController.shared.isUsingPadUI {
                ...
                cell.coinTypeTextField.alpha = 0.3
                cell.purposeTextField.isUserInteractionEnabled = false
                cell.purposeTextField.alpha = 0.3
                if self.recoveryCryptocurrencies.count > indexPath.section {
                    ...
                    if self.recoveryCryptocurrencies[indexPath.section].currency == .btc || self.recoveryCryptocurrencies[indexPath.section].currency == .ltc {
                        cell.purposeTextField.isUserInteractionEnabled = true
                        cell.purposeTextField.alpha = 1
                    }
                }
                tableViewCell = cell
            }
    ...
    func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        ...
        case .purpose:
            return self.segwitPurposeItems.count
        }
    }
    ...
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        ...
        case .year:
            return "\(2009 + row)"
        case .purpose:
            return self.segwitPurposeItems[row]
        }
    }
